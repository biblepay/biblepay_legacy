...
void OverviewPage::updateAdvancedPSUI(bool fShowAdvancedPSUI) {
    ...
    if (fMasternodesEnabled) ui->privateSendInfo->setVisible(true);
...
void OverviewPage::privateSendStatus()
{
    ...
    if((fMasternodesEnabled && !masternodeSync.IsBlockchainSynced()) || ShutdownRequested()) return;
...
void OverviewPage::DisablePrivateSendCompletely() 
{
	...
	//Biblepay: Disable the Start Mixing and Info buttons until the Sanctuaries go Live:
