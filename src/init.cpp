...
void CheckGenesisBlock(CBlock bl1, uint256 targetBlockHash, uint256 targetMerkleRoot)
{
	    
	    printf("block.nTime = %u \n", bl1.nTime);
        printf("block.nNonce = %u \n", bl1.nNonce);
        printf("block.GetHash = %s\n", bl1.GetHash().ToString().c_str());
		printf("block.hashMerkleRoot = %s\n", bl1.hashMerkleRoot.ToString().c_str());
		printf("BlockMerkleRoot(block) = %s\n",  BlockMerkleRoot(bl1).ToString().c_str());
		printf("block hash target = %s\n", targetBlockHash.ToString().c_str());

        if (((bl1.GetHash() != targetBlockHash) || (BlockMerkleRoot(bl1) != targetMerkleRoot)))
	    {
            printf("Searching for genesis block...\n");
			CBlock block = CreateGenesisBlock(bl1.nTime,bl1.nNonce,bl1.nBits,bl1.nVersion,50 * COIN);
            ...
            printf("Solution block.nTime = %u \n", block.nTime);
            printf("Solution block.nNonce = %u \n", block.nNonce);
            printf("Solution block.GetHash = %s\n", block.GetHash().ToString().c_str());
			printf("Solution block.hashMerkleRoot = %s\n", block.hashMerkleRoot.ToString().c_str());
			printf("Solution BlockMerkleRoot(block) = %s\n",  BlockMerkleRoot(block).ToString().c_str());

			// End program here
			return;
    }

    assert(bl1.GetHash() == targetBlockHash); // Verify the genesis block hash matches the wallet's genesis block hash
    assert(BlockMerkleRoot(bl1) == targetMerkleRoot);
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, mempoolrej, net, proxy, prune, http, libevent, tor, zmq, "
                             "biblepay (or specifically: privatesend, instantsend, masternode, spork, keepass, mnpayments, gobject)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N biblepay anonymized (default: %u)"), DEFAULT_PRIVATESEND_AMOUNT));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2014-%i The Biblepay Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Biblepay Core version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}

/** Initialize Biblepay Core.
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Biblepay Core is shutting down."));
    ...
    try {
        ...
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Biblepay Core is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Biblepay Core is probably already running.") + " %s.", strDataDir, e.what()));
    }
	...
	strSubVersion = FormatSubVersion(CLIENT_NAME, BIBLE_VERSION, uacomments);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                if (!mapBlockIndex.empty() && mapBlockIndex.count(cblockGenesis.GetHash()) == 0)
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Biblepay Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Biblepay Core to complete") << "\n";
    ...
    // ********************************************************* Step 11c: update block tip in Biblepay modules
    ...
    // ********************************************************* Step 11d: start biblepay-privatesend thread
	...
	threadGroup.create_thread(boost::bind(&MemorizeBlockChainPrayers, false, true));
