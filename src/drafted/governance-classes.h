 ...
 *
 *   // network
 *   CBiblepayNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.biblepay.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CBiblepayNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CBiblepayNetwork(UniValue objIn)
    {
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        // TO DO : For testnet version bump
...
// // can be under: BiblepayNetwork
// //   -- signature requirements : Key1(User)
// class CBiblepayNetworkVariable : public CGovernanceObject
// {
// private:
...
//     template <typename Stream, typename Operation>
//     virtual inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
//         // TO D O : For testnet version bump
//         READWRITE(nGovernanceType);
