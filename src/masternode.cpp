...
bool CMasternodeBroadcast::Update(CMasternode* pmn, int& nDos)
{
    ...
    if(!pmn->IsBroadcastedWithin(MASTERNODE_MIN_MNB_SECONDS) || (fMasterNode && pubKeyMasternode == activeMasternode.pubKeyMasternode)) {
        ...
        if (fDebugMaster) LogPrintf("CMasternodeBroadcast::Update -- Got UPDATED Masternode entry: addr=%s\n", addr.ToString());
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
    ...
    // should be at least not earlier than block when 1,550,001 biblepay tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for 500,000 biblepay tx -> 1 confirmation
