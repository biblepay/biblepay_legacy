...
std::string CKeePassIntegrator::constructHTTPPost(const std::string& strMsg, const std::map<std::string,std::string>& mapRequestHeaders)
{
    ...
    streamOut << "POST / HTTP/1.1\r\n"
      << "User-Agent: biblepay-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: localhost\r\n"
...
//    }
    // Create event base
    struct event_base *base = event_base_new(); // TO DO RAII
    if (!base)
        throw std::runtime_error("cannot create event_base");

    // Synchronously look up hostname
    struct evhttp_connection *evcon = evhttp_connection_base_new(base, NULL, KEEPASS_HTTP_HOST, DEFAULT_KEEPASS_HTTP_PORT); // TO DO RAII
    if (evcon == NULL)
...
//    std::string strPost = constructHTTPPost(sRequest, mapRequestHeaders);
    ...
    HTTPReply response;
    struct evhttp_request *req = evhttp_request_new(http_request_done, (void*)&response); // TO DO RAII
    if (req == NULL)
...
//    s << "POST / HTTP/1.1\r\n"
    evhttp_add_header(output_headers, "User-Agent", ("biblepay-json-rpc/" + FormatFullVersion()).c_str());
...
void CKeePassIntegrator::rpcSetLogin(const SecureString& sWalletPass, const SecureString& sEntryId)
{
    ...
    request.addStrParameter("Login", SecureString("biblepay"));
