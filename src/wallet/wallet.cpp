...
{
    // TO DO: fix handling of 'change' outputs. The assumption is that any
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate)
{
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex)
        {
            ...
            CBlock block;
            ReadBlockFromDisk(block, pindex, Params().GetConsensus(), "SCANFORWALLETTRANSACTIONS");
        ...
        if (pcoin->IsTrusted())
		{
		    ...
		    for (unsigned int i = 0; i < pcoin->vout.size(); i++)
			{
				if (!IsSpent(hash, i))
				{
					...
					CAmount nAmount = GetCredit(txout, ISMINE_SPENDABLE);
					if (nAmount > (.001*COIN))
					{
						nTotal += nAmount;
	...
	CAmount caMinCoinAmount, CAmount nBankrollMask) const
{
    ...
    BOOST_FOREACH(const COutPoint& outpoint, vPresetInputs)
    {
        ...
        if (it != mapWallet.end())
        {
            ...
            return false; // TO DO: Allow non-wallet inputs
    }
	...
	int nPrivateSendRoundsMin, int nPrivateSendRoundsMax)
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        //if(out.tx->vout[out.i].nValue == SANCTUARY_COLLATERAL*COIN) continue;
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // TO DO: pass in scriptChange instead of reservekey so
...
{
    {
        ...
        LogPrintf("CommitTransaction:\n%s", wtxNew.ToString());
				LogPrintf("committing Tx with message %s \r\n",wtxNew.vout[0].sTxOutMessage.c_str());
