...
static const CAmount nHighTransactionFeeWarning = 0.20 * COIN;
//! -fallbackfee default
...
static void ReadOrderPos(int64_t& nOrderPos, mapValue_t& mapValue)
{
    if (!mapValue.count("n"))
    {
        nOrderPos = -1; // TO DO: calculate elsewhere
        return;
    }
...
public:
    ...
    //! to do: add something to note what created it (user, getnewaddress, change)
...
private:
    ...
    bool SelectCoins(const CAmount& nTargetValue, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl *coinControl = NULL, 
		AvailableCoinsType nCoinType=ALL_COINS, bool fUseInstantSend = true, int iMinConfirms = 0, double dMinCoinAge = 0, CAmount caMinCoinAmount = 0, CAmount nBankRollMask = 0, 
		std::string sSpecificTxId = "", int nSpecificOutput = 0) const;
...
public:
    ...
    void AvailableCoins(std::vector<COutput>& vCoins, bool fOnlyConfirmed=true, const CCoinControl *coinControl = NULL, 
		...
		double nMinCoinAge = 0, CAmount nMinCoinAmount = 0, CAmount nBankRollMask = 0, std::string sSpecificTxId = "", int nSpecificOutput = 0) const;
    ...
    /// Get 500,000 Biblepay output and keys which can be used for the Masternode
    ...
    std::map<int64_t, CTitheObject> GetDimensionalCoins(double nMinAge, CAmount nMinAmount, double nMaxAge=9999999, CAmount nMaxAmount=MAX_MONEY) const;
    ...
    bool CreateTransaction(const std::vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, int& nChangePosRet,
						   ...
						   CAmount nBankRollMask = 0, std::string sSpecificTxId = "", int nSpecificOutput = 0);
